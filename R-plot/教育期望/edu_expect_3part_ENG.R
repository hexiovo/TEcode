...
long_data$Group <- factor(long_data$Group, levels = 1:3, 
                          labels = c("Constructive Learning", "Passive Learning", "Active Learning"))
# 将变量名替换为中文
long_data <- long_data %>%
  mutate(Condition = case_when(
    Condition == "Memory_T1" ~ "Memory Test",
    Condition == "Understand_T1" ~ "Understand Test",
    Condition == "Trans_T1" ~ "Transfer Test"
  ))
...
# 绘制
p <- ggplot(summary_data, aes(x = Group, y = Mean, fill = factor(Group))) +
  geom_bar(stat = "identity", position = "dodge", width = 0.6) +
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), 
                position = position_dodge(width = 0.7), 
                width = 0.1) +
  labs(x = "Test Condition", y = "Test Score", fill = "Group Condition") +
  theme_minimal() +
  theme(
    # 图例
...
# 添加显著性标记
p <- p + 
  # 在图1（Memory Test）添加第一个显著性标记
  geom_signif(data = summary_data %>% filter(Condition == "Memory Test"), 
              aes(xmin = 1, xmax = 2, annotations = "*", y_position = 18), 
              textsize = 8, vjust = 0.5, tip_length = c(0.04, 0.04), manual = TRUE) +
  
  # 在图2（Understand Test）添加两个显著性标记
  geom_signif(data = summary_data %>% filter(Condition == "Understand Test"), 
              aes(xmin = 1, xmax = 3, annotations = "*", y_position = 18), 
              textsize = 10, vjust = 0.5, tip_length = c(0.04, 0.04), manual = TRUE) +
  
  geom_signif(data = summary_data %>% filter(Condition == "Understand Test"), 
              aes(xmin = 1, xmax = 2, annotations = "**", y_position = 17), 
              textsize = 10, vjust = 0.5, tip_length = c(0.04, 0.04), manual = TRUE)


print(p)
grid_xposition <- 0.0337
grid_yosition <- 0.988
# 在第一个分面 (Memory Test) 的 y 轴尽头处添加一个箭头
grid.text("∧", x = unit(grid_xposition ,"npc"), y = unit(grid_yosition, "npc"), gp = gpar(col = "black", fontsize = 30))