...
#转换列模式为行模式，一对一
long_data <- melt(all_data, id.vars = "Group", measure.vars = c("Score_T1"),
...
long_data <- long_data %>%
  mutate(Condition = case_when(
    Condition == "Score_T1" ~ "Total Score",
  ))
...
p <- ggplot(summary_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  coord_fixed(ratio = 0.11)  # 设置纵横比，压缩纵向比例

p <- p + 
  geom_signif(data=summary_data, aes(xmin=1, xmax=3, annotations="*", y_position=40), 
              textsize = 10, vjust = 0.5, tip_length = c(0.04, 0.04), manual=TRUE) +
  geom_signif(data=summary_data, aes(xmin=1, xmax=2, annotations="***", y_position=37), 
              textsize = 10, vjust = 0.5, tip_length = c(0.04, 0.04), manual=TRUE) 

print(p)
# 在第一个分面 (Memory Test) 的 y 轴尽头处添加一个箭头
grid_xposition <- 0.3556
grid_yposition <- 0.988
