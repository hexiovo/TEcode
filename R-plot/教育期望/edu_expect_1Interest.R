...
#转换列模式为行模式，一对一
long_data <- melt(all_data, id.vars = "Group", measure.vars = c("Interest"),
...
long_data <- long_data %>%
  mutate(Condition = case_when(
    Condition == "Interest" ~ "Interest Level",
  ))
...
p <- ggplot(summary_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), 
                ...
                width = 0.1) +
  labs(x = "Test Condition", y = "Interest Level", fill = "Group Condition") +
  theme_minimal() +
  theme(
    #去掉图例
  ...
  coord_fixed(ratio = 0.85)  # 设置纵横比，压缩纵向比例

p <- p + 
  geom_signif(data=summary_data, aes(xmin=2, xmax=3, annotations="*", y_position=5), 
              textsize = 10, vjust = 0.5, tip_length = c(0.06, 0.06), manual=TRUE) +
  geom_signif(data=summary_data, aes(xmin=1, xmax=2, annotations="**", y_position=5), 
            textsize = 10, vjust = 0.5, tip_length = c(0.06, 0.06), manual=TRUE) 
print(p)
# 在第一个分面 (Memory Test) 的 y 轴尽头处添加一个箭头
grid_xposition <- 0.347
