...
#转换列模式为行模式，一对一
long_data <- melt(all_data, id.vars = "Group", measure.vars = c("Trans_between_cata"),
...
long_data <- long_data %>%
  mutate(Condition = case_when(
    Condition == "Trans_between_cata" ~ "Trans between Relate",
  ))
...
p <- ggplot(summary_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), 
                ...
                width = 0.1) +
  labs(x = "Test Condition", y = "Trans between Relate", fill = "Group Condition") +
  theme_minimal() +
  theme(
    #去掉图例
  ...
  coord_fixed(ratio = 0.011)  # 设置纵横比，压缩纵向比例

p <- p + 
  geom_signif(data=summary_data, aes(xmin=1, xmax=3, annotations="**", y_position=330), 
