...
# 假设数据框有列 "ISC_xy"、"Understand_T1" 和 "Group"
# 更改 "Group" 列为因子
...
results <- all_data %>%
  group_by(Group) %>%
  summarize(
    r = cor(ISC_xy, Understand_T1, use = "complete.obs"),
    .groups = 'drop'
  )
...
# 绘制图形
p <- ggplot(all_data, aes(x = ISC_xy, y = Understand_T1, color = Group)) +
  geom_point() +
  geom_smooth(method = "lm", aes(fill = Group), alpha = 0.2) +
  labs(
    title = "Understand Score vs ISC",
    x = "ISC",
    y = "Understand Score"
  ) +
  theme_minimal() +
  ...
  geom_text(data = results, aes(x = 0.75, y = 13.5,  
  ...
  scale_y_continuous(limits = c(4, NA), expand = expansion(mult = c(0, 0.05)))  # 强制Y从0开始

# 打印图形
print(p)

# 在图形中添加箭头
grid_xposition <- 0.0625  # 根据需要调整箭头的X位置
