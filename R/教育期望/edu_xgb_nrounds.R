...
average_accuracy_per_nrounds <- numeric(16)  # 用于存储不同迭代次数的平均准确率

# 修改迭代次数范围为50到200，步长为10
for (num_nrounds in seq(50, 200, by = 10)) {
  ...
  average_accuracy_per_nrounds[(num_nrounds - 40) / 10] <- average_accuracy_xgb  # 对应索引 1-16
  
  # 输出每个迭代次数的平均准确率
  cat("迭代次数", num_nrounds, "的平均准确率:", average_accuracy_xgb, "\n")
}

# 最终输出所有迭代次数的平均准确率
average_accuracy_per_nrounds

# 创建数据框，nrounds 从 50 到 200
df <- data.frame(nrounds = seq(50, 200, by = 10), 
                 accuracy = average_accuracy_per_nrounds)

# 绘制折线图
ggplot(df, aes(x = nrounds, y = accuracy)) +
  geom_line(color = "blue", size = 1) +  # 折线
  geom_point(color = "red", size = 2) +  # 数据点
  labs(title = "Average Accuracy per Nrounds", 
       x = "Nrounds (50-200)", 
       y = "Accuracy") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +  # 标题居中
  scale_x_continuous(breaks = seq(50, 200, by = 10))  # 设置横坐标刻度从 50 到 200
