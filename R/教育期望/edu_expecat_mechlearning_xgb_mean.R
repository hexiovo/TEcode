...
iteration_times <- 10
# 读取数据
xlsx_path <- "F:/桌面/文件/实验/教育期望/数据处理/可用数据-5.xlsx"
all_data <- read_excel(xlsx_path, sheet = 1)

# 初始化准确率列表和特征得分列表
accuracy_list <- numeric(iteration_times)
feature_scores <- rep(0, length(c(10:25, 29)))  # 与特征数量相同的向量
# 定义你要修改的列的索引
columns_to_rename <- c(10:25, 29)

# 定义新的列名标签
new_labels <- c( "ISC_ts", "ISC_ss", "P_word", "P_picture", 
                 "C_AP", "T_AP-Dur", "MT_AP-Dur", 
                 "T_Total", "T_b-Cata", "M_PupilFix", 
                 "T_B-Relate", "PT_Total", "PT_B-Cata", 
                 "M_Dur", "M_Scan", "Stress", "Interest")
...
# 手动设置优先顺序
feature_ranking_df$Score[feature_ranking_df$Feature == "ISC"] <- feature_ranking_df$Score[feature_ranking_df$Feature == "ISC"] + 0.00001
...
ggplot(feature_ranking_df, aes(x = reorder(Feature, -Score), y = Score, fill = Score)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 12, vjust = 5), #增大X标签字体
    plot.title = element_text(size = 20, hjust = 0.5),  # 将标题居中,放大
    ...
    legend.text = element_text(size = 16)  # 增大图例文本字体
  )
cat("\014")
